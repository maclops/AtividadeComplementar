...
public class AlunoResourceTest {
	...
	public void teste01RequisicaoIdSucesso() throws Exception {
		...
		this.mvc.perform(get(url)).andExpect(status().isOk()).andExpect(jsonPath("nome", equalTo("carlos eduardo")));
	}
	...
	public void teste05RequisicaoFindAll() throws Exception {
		...
		this.mvc.perform(get(url)).andExpect(status().isOk()).andExpect(content().string(containsString("ra")));
	}
	...
	public void teste08Post() throws Exception {
		...
		this.mvc.perform(
				post(url).content("{\"ra\":54321,\"nome\":\"gabriel\"}").contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isCreated())
				.andExpect(header().string("Location", is("http://localhost/alunos/123456")))
				.andExpect(content().string("")).andDo(MockMvcResultHandlers.print());
	}

	@Test
	public void teste09PostIncompletoNome() throws Exception {
		String url = "/alunos/";
		this.mvc.perform(post(url).content("{\"ra\":54321}").contentType(MediaType.APPLICATION_JSON))
	...
	public void teste10PostIncompletoRa() throws Exception {
		String url = "/alunos/";
		this.mvc.perform(post(url).content("{\"nome\":\"gabriel\"}").contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isBadRequest()).andDo(MockMvcResultHandlers.print());
	}
	...
	public void teste11PutOk() throws Exception {
		...
		this.mvc.perform(
				put(url).content("{\"ra\":54321,\"nome\":\"gabriel guilherme\"}").contentType(MediaType.APPLICATION_JSON))
	...
	public void teste12PutIncompletoNome() throws Exception {
		String url = "/alunos/";
		this.mvc.perform(put(url).content("{\"ra\":54321}").contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isBadRequest()).andDo(MockMvcResultHandlers.print());
	}
